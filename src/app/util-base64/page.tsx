...
export default function Page() {
  ...
  function doEncode() {
    try {
      ...
      setStatus("error"); setErrorMsg(e.message || t("util_base64.error_encode"));
    }
  }
  ...
  function doDecode() {
    try {
      ...
      setStatus("error"); setErrorMsg(e.message || t("util_base64.error_decode"));
    }
  }
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8">
      <header className="mb-10 animate-fade-in">
        ...
        <h1 className="font-display text-3xl font-bold tracking-tight text-zinc-900 sm:text-4xl">{t("util_base64.title")}</h1>
        <p className="mt-2 max-w-xl text-sm leading-relaxed text-zinc-500">{t("util_base64.支持UTF8编码")}</p>
      </header>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200/70 bg-white shadow-soft px-5 py-3">
        <span className="text-xs font-medium text-zinc-600">{t("util_base64.label_encoding")}</span>
        <div className="flex rounded-lg bg-slate-100 p-1 text-xs">
          <button onClick={() => setMode("utf8")} className={`rounded-md px-3 py-1 ${mode === "utf8" ? "bg-white text-brand-600 shadow-sm font-medium" : "text-slate-500"}`}>UTF-8 {t("util_base64.recommended")}</button>
        ...
        <span className="text-[11px] text-zinc-400">{t("util_base64.utf8_hint")}</span>
      </div>
        ...
        <section className="rounded-2xl border border-slate-200/70 bg-white shadow-soft">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-zinc-800">{t("util_base64.input_title")}</h2>
          ...
          <textarea
            ...
            placeholder={mode === "utf8" ? t("util_base64.placeholder_input") : t("util_base64.placeholder_latin1")}
            spellCheck={false}
        ...
        <section className="rounded-2xl border border-slate-200/70 bg-white shadow-soft">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-zinc-800">{t("util_base64.output_title")}</h2>
            </div>
          ...
          <textarea
            value={output}
            readOnly
            placeholder={t("util_base64.placeholder结果将显示在这里")}
            spellCheck={false}
            className="h-[380px] w-full resize-none bg-transparent px-5 py-4 font-mono text-[13px] text-zinc-800 placeholder:text-zinc-300 focus:outline-none scrollbar-thin"
          />
        </section>
